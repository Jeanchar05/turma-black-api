...
must(exists("public/assets/login/primo-login-dark-v34.webp"), "Arte dark do Login/Roleta ausente");
must(exists("public/assets/login/primo-login-light-v34.webp"), "Arte light do Login/Roleta ausente");
...
const notifications = read("public/dashboard-notifications-live.js");
...
must(notifications.includes("Push em segundo plano não configurado"), "Push precisa ter fallback sem quebrar notificações internas");
const nav = read("public/page-navigation.js");
must(nav.includes("dashboard-notifications-live.js?v=20260923-v17"), "Notificações precisam carregar no workspace do aluno");
...
console.log("Platform V17 regression: OK");
