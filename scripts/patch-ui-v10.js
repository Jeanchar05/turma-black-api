...
if (start >= 0 && end >= 0) {
  ...
  resultado = resultado
    ...
    .replace(/<link[^>]+data-ui-v(?:10|11|12|13|14|15|16)[^>]*>\\s*/gi, "")
    .replace(/<script[^>]+data-ui-v(?:10|11|12|13|14|15|16)[^>]*><\\/script>\\s*/gi, "");
  ...
  console.error("[UI V16] Não foi possível localizar aplicarCamadaResponsiva em server.js.");
  process.exit(1);
}

source = source.replace(/res\.setHeader\("X-UI-Version", "[^"]+"\);/g, `res.setHeader("X-UI-Version", "${UI_LABEL}");`);
if (!source.includes("X-UI-Version")) {
  source = source.replace('res.setHeader("X-Cache-Version", CACHE_VERSION);', `res.setHeader("X-Cache-Version", CACHE_VERSION);\n    res.setHeader("X-UI-Version", "${UI_LABEL}");`);
}
source = source.replace(/release: "[^"]+",(?:\n\s*uiVersion: "[^"]+",)?/, `release: "${UI_LABEL}",\n    uiVersion: "${UI_LABEL}",`);

if (source !== original) {
  fs.writeFileSync(serverPath, source, "utf8");
  console.log(`[UI V16] server.js atualizado para ${UI_VERSION}.`);
} else {
  console.log(`[UI V16] servidor já estava atualizado para ${UI_VERSION}.`);
}
