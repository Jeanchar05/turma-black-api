...
const UI_VERSION = "20260803-imperial-v14-final-1";
const UI_LABEL = "imperial-v14-final-1";

if (!fs.existsSync(serverPath)) {
  console.error("[UI V14] server.js não encontrado.");
  process.exit(1);
}
...
if (start >= 0 && end >= 0) {
  ...
  const folhasLegadas = [
    ...
    "turma-unified-v12", "turma-obsidian-v13", "turma-imperial-v14"
  ].join("|");
  ...
  resultado = resultado
    ...
    .replace(/<link[^>]+data-ui-v(?:10|11|12|13|14)[^>]*>\\s*/gi, "")
    .replace(/<script[^>]+data-ui-v(?:10|11|12|13|14)[^>]*><\\/script>\\s*/gi, "");

  const camadaV14 = \`
  <link rel="stylesheet" href="/responsive-global.css?v=${UI_VERSION}" data-global-responsive />
  <link rel="stylesheet" href="/turma-imperial-v14.css?v=${UI_VERSION}" data-ui-v14="css" />
  <script defer src="/turma-imperial-v14.js?v=${UI_VERSION}" data-ui-v14="js"></script>\`;

  resultado = resultado.replace("</head>", camadaV14 + "\\n</head>");
  return resultado;
}`;

  source = source.slice(0, start) + replacement + source.slice(end + 2);
} else {
  console.error("[UI V14] Não foi possível localizar aplicarCamadaResponsiva em server.js.");
  process.exit(1);
}
...
if (source !== original) {
  ...
  console.log(`[UI V14] server.js atualizado para ${UI_VERSION}.`);
} else {
  console.log(`[UI V14] servidor já estava atualizado para ${UI_VERSION}.`);
}
