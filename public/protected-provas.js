...
(() => {
  ...
  if(!document.querySelector('script[data-global-theme-v2]')){
  ...
  function forceVisible() {
    ...
    if (loader && !document.body.classList.contains("protected-ready")) {
  ...
  function clearSession() {
    for (const storage of [sessionStorage, localStorage]) {
      for (const key of KEYS) { try { storage.removeItem(key); } catch (_) {} }
    }
  }
  ...
  function release(user) {
    ...
    document.getElementById("examsLoading")?.remove();
    document.dispatchEvent(new CustomEvent("turma:protected-ready", { detail: { user } }));
    document.dispatchEvent(new CustomEvent("turma:exams-ready", { detail: { user } }));
  }
