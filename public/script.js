...
(() => {
  const API_URL = (() => {
    ...
    return local ? "https://turma-black-api-es3u.onrender.com" : window.location.origin;
  })();
  ...
  function limparLocalStorageAntigo() {
    const chaves = [...TOKEN_KEYS, "usuario", "usuarioLogado", "user", "currentUser", "perfil", "cargo", "plano"];
  ...
  async function api(endpoint, { method = "GET", body = null, auth = false, timeout = 18000 } = {}) {
    ...
    try {
      ...
      catch (_) { dados = { mensagem: texto || "Resposta inválida do servidor." }; }

      if (!resposta.ok || dados.erro) {
        throw new Error(dados.erro || dados.mensagem || dados.message || `Erro ${resposta.status}.`);
      }
      return dados;
    } catch (erro) {
      if (erro.name === "AbortError") throw new Error("O servidor demorou para responder. Tente novamente.");
      if (erro instanceof TypeError) throw new Error("Não foi possível conectar ao servidor.");
      throw erro;
    } finally {
      clearTimeout(timer);
    }
  ...
  function mascaraTelefone(valor) {
    ...
    if (numero.length <= 10) {
      return numero.replace(/^(\d{2})(\d)/, "($1) $2").replace(/(\d{4})(\d)/, "$1-$2");
    }
    return numero.replace(/^(\d{2})(\d)/, "($1) $2").replace(/(\d{5})(\d)/, "$1-$2");
  }

  function cargo(usuario) {
    return String(usuario?.cargo || usuario?.tipo || usuario?.role || usuario?.perfil || "")
      .trim().toLowerCase().replaceAll("_", "-").replace(/\s+/g, "-");
  }

  function planoAtivo(usuario) {
    if (!usuario) return false;
    const c = cargo(usuario);
    if (["admin", "superadmin", "super-admin", "moderador", "suporte", "vendedor"].includes(c)) return true;
    if (usuario.suspenso || ["suspenso", "bloqueado"].includes(usuario.status) || usuario.aprovado === false) return false;

    const plano = String(usuario.plano || "").trim().toLowerCase();
    if (!["premium", "black", "black30", "black90", "black180", "black360", "admin"].includes(plano)) return false;
  ...
  function acessoEspecial(usuario) {
    const c = cargo(usuario);
    return ["admin", "superadmin", "super-admin", "moderador", "suporte", "vendedor"].includes(c) ||
      usuario?.vendedor === true ||
      usuario?.acessosRapidos?.painelAdmin === true ||
      usuario?.acessosRapidos?.painelVendas === true;
  }

  function configurarCards(usuario) {
    const c = cargo(usuario);
    const acessos = usuario?.acessosRapidos || {};
    const vendedor = usuario?.vendedor === true || c === "vendedor";
    ...
    if (admin) admin.hidden = !(acessos.painelAdmin === true || ["superadmin", "super-admin", "admin", "moderador"].includes(c));
    if (vendas) vendas.hidden = !(acessos.painelVendas === true || vendedor || ["superadmin", "super-admin", "admin", "suporte"].includes(c));
  }
  ...
  function redirecionar(usuario) {
    ...
    if (acessoEspecial(usuario)) {
      ocultarLoading();
      abrirEscolha(usuario);
      return;
    }
    window.location.href = planoAtivo(usuario) ? "dashboard.html" : "dashboard-free.html";
  }
  ...
  function mostrarMensagem(texto, tipo = "erro") {
    ...
    box._timer = setTimeout(() => box.classList.remove("active"), 4800);
  }

  function mostrarLoading(texto = "Preparando seu acesso…") {
    const overlay = $("authLoadingOverlay");
    if (!overlay) return;
    if ($("authLoadingTexto")) $("authLoadingTexto").textContent = texto;
    overlay.hidden = false;
    overlay.setAttribute("aria-hidden", "false");
    requestAnimationFrame(() => overlay.classList.add("ativo", "active", "open"));
  }
  ...
  function botaoCarregando(botao, ativo) {
    ...
    if (botao.dataset.original) botao.innerHTML = botao.dataset.original;
  }
  ...
  function ativarLogin() {
    ...
    $("authTitulo").textContent = "Bem-vindo(a) de volta!";
  ...
  function ativarCadastro() {
    ...
    $("authTitulo").textContent = "Crie sua conta grátis";
  ...
  function atualizarForca() {
    ...
    if (!input.value) { texto.textContent = "Digite uma senha"; return; }
  ...
  async function login(evento) {
    ...
    if (!emailValido(email)) { mostrarMensagem("Digite um e-mail válido."); $("loginEmail")?.focus(); return; }
    if (senha.length < 4) { mostrarMensagem("A senha precisa ter pelo menos 4 caracteres."); $("loginSenha")?.focus(); return; }

    botaoCarregando(botao, true);
    mostrarLoading("Verificando suas credenciais…");

    try {
      const resposta = await api("/login", { method: "POST", body: { email, senha } });
      if (!resposta?.token) throw new Error("O servidor não retornou um token de acesso.");
      ...
      setTimeout(() => redirecionar(me.usuario), 450);
    } catch (erro) {
      limparSessao();
      ocultarLoading();
  ...
  async function criar(evento) {
    ...
    if (nome.length < 3) { mostrarMensagem("Digite seu nome completo."); $("cadastroNome")?.focus(); return; }
    if (!emailValido(email)) { mostrarMensagem("Digite um e-mail válido."); $("cadastroEmail")?.focus(); return; }
    if (telefone.replace(/\D/g, "") && telefone.replace(/\D/g, "").length < 10) { mostrarMensagem("Digite o WhatsApp corretamente."); $("cadastroTelefone")?.focus(); return; }
    if (senha.length < 6) { mostrarMensagem("A senha precisa ter pelo menos 6 caracteres."); $("cadastroSenha")?.focus(); return; }

    botaoCarregando(botao, true);
    mostrarLoading("Criando sua conta gratuita…");

    try {
      const resposta = await api("/criar", { method: "POST", body: { nome, email, telefone, senha } });
      if (!resposta?.sucesso) throw new Error(resposta?.mensagem || "Não foi possível criar a conta.");
      ocultarLoading();
      evento.currentTarget.reset();
      atualizarForca();
      ativarLogin();
      if ($("loginEmail")) $("loginEmail").value = email;
      mostrarMensagem("Conta criada com sucesso. Agora faça seu login.", "sucesso");
  ...
  function registrarEventos() {
    ...
    $("formCadastro")?.addEventListener("submit", criar);
    $("cadastroSenha")?.addEventListener("input", atualizarForca);
    $("cadastroTelefone")?.addEventListener("input", (evento) => { evento.target.value = mascaraTelefone(evento.target.value); });
    $$(".toggle-password").forEach((botao) => botao.addEventListener("click", () => alternarSenha(botao)));
    $$('[data-go]').forEach((botao) => botao.addEventListener("click", () => { if (!botao.hidden && !botao.disabled) window.location.href = botao.dataset.go; }));
    $("fecharAdminChoice")?.addEventListener("click", fecharEscolha);
    $("adminChoiceOverlay")?.addEventListener("click", (evento) => { if (evento.target === evento.currentTarget) fecharEscolha(); });
    document.addEventListener("keydown", (evento) => { if (evento.key === "Escape") fecharEscolha(); });
  }
})();
