...
(() => {
  ...
  function isCurrentItem(key){const path=normalizedPath((window.TurmaNavigation?.pathname ?? location.pathname));if(MENU[key]?.paths.includes(path))return true;return key==="estudo"&&path.startsWith("/estudo-")}
  ...
  function removeStudentAdminLinks(sidebar){if(!sidebar||/^\/admin(?:\/|$)/.test((window.TurmaNavigation?.pathname ?? location.pathname)))return;sidebar.querySelectorAll("a[href],button[data-nav]").forEach(element=>{const href=element.getAttribute("href");const path=href?normalizedPath(href):"";const dataNav=String(element.dataset?.nav||"").toLowerCase();const isAdmin=ADMIN_PATHS.includes(path)||["admin","painel-admin","painel-vendas","relatorios","usuarios","configuracoes"].includes(dataNav);if(isAdmin)element.remove()});sidebar.querySelectorAll("h2,h3,h4,small,span,p").forEach(element=>{if(/^administra[cç][aã]o$/i.test(element.textContent.trim())){const parent=element.closest("section,.nav-section,.menu-section")||element;if(!parent.querySelector?.("a[href],button[data-nav]"))parent.remove()}})}

  function ensureGestaoItem(nav){if(!nav||ADMIN_PATHS.includes(normalizedPath((window.TurmaNavigation?.pathname ?? location.pathname))))return;const recognized=Array.from(nav.children).filter(element=>itemKey(element));if(recognized.length<5||recognized.some(element=>itemKey(element)==="gestao"))return;const template=recognized.find(element=>element.classList.contains("dash-nav-item"))||recognized[0];const item=document.createElement("a");item.className=template?.className||"dash-nav-item";item.classList.remove("active");item.href="/gestao";item.dataset.nav="gestao";item.innerHTML='<span class="tp-management-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20V10M10 20V4M16 20v-7M22 20H2M3 10l6-6 6 9 6-7"/></svg></span><b>Gestão</b>';nav.appendChild(item)}
