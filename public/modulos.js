...
(() => {
  ...
  function readHash(){const parts=(window.TurmaNavigation?.hash||location.hash).slice(1).split("/");if(["modulos","instagram","pdf"].includes(parts[0]))tab=parts[0];if(C.modules.some(m=>m.id===parts[1]))selected=parts[1];}
  ...
  function instagramAction(m){const a=m.action;if(!a||a.kind==="none")return "";const related=catalog.modules.find(v=>v.id===a.moduleId);const href=a.kind==="study"&&related?`/estudo-${related.route}`:a.kind==="pdf"&&related?`/modulos#pdf/${related.id}`:a.kind==="link"&&/^https:\/\//.test(a.url)?a.url:"";return href?`<a class="lib-instagram-action" href="${escape(href)}" ${a.kind==="link"?'target="_blank" rel="noopener noreferrer"':""}>${escape(a.label||(a.kind==="study"?"Praticar no estudo":a.kind==="pdf"?"Revisar o PDF":"Saiba mais"))} →</a>`:"";}
