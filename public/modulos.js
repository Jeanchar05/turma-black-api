...
(() => {
  const C = window.TurmaStudy,
    S = window.TurmaStudySync,
    Media = window.TurmaMedia;
  ...
  let catalog,
    ...
    opener;
  const current = () =>
    catalog.modules.find((m) => m.id === selected) || catalog.modules[0];
  const videoProgress = (m) => {
    const v = S.state.videos?.[m.id];
    return v?.source === m.url ? v : null;
  };
  const art = (m) =>
    `/assets/modules-v4/${m.art}-${document.documentElement.dataset.theme === "light" ? "light" : "dark"}.webp`;
  const cover = (m, extra = "") =>
    `<img src="${art(m)}" data-art="${m.art}" alt="${escape(m.title)}" loading="lazy" width="480" height="320" ${extra}>`;
  function headers() {
    const h = {
      "Content-Type": "application/json",
      Accept: "application/json",
    };
    if (user) h["X-Study-Account"] = String(user.id || user._id);
  ...
  async function api(url, options = {}) {
    ...
    if (!response.ok)
      throw Error(
        data.erro || "Não foi possível carregar agora. Tente novamente.",
      );
    return data;
  }
  ...
  function applyTheme() {
    ...
    document.querySelector('meta[name="theme-color"]').content = light
      ? "#f3f1f7"
      : "#0b0b11";
    all("[data-art]").forEach(
      (img) =>
        (img.src = `/assets/modules-v4/${img.dataset.art}-${light ? "light" : "dark"}.webp`),
    );
  }
  ...
  document.addEventListener("keydown", (e) => {
    if (document.body.classList.contains("menu-open")) {
      ...
      if (e.key === "Tab") {
        const buttons = all("a,button", $("studySidebar")).filter(
  ...
  function readHash() {
    const parts = (window.TurmaNavigation?.hash || location.hash)
      .slice(1)
      .split("/");
    if (["modulos", "instagram", "pdf"].includes(parts[0])) tab = parts[0];
    if (C.modules.some((m) => m.id === parts[1])) selected = parts[1];
  }
  function navigate(next, id = selected, scroll = false) {
    stopPlayer();
    tab = next;
    selected = id;
    const route = window.TurmaNavigation?.pathname || "/modulos";
    history.replaceState(history.state, "", `${route}#${tab}/${selected}`);
    $("librarySearch").value = "";
    $("libraryFilter").value = "all";
    renderTab();
    if (scroll)
      $("libraryTabs").scrollIntoView({ behavior: "smooth", block: "start" });
  }
  function render() {
    $("modulesApp").innerHTML =
      `<section class="lib-hero"><div><span class="learn-eyebrow">SUA CENTRAL DE CONHECIMENTO</span><h1>O próximo passo<br>começa com um <em>play.</em></h1><p>Assista à explicação. Revise os pontos importantes.<br>Leve o aprendizado para a prática.</p><div class="lib-hero-stats"><span><strong>8</strong> módulos de estudo</span><span><strong>${catalog.modules.filter((m) => m.published).length}</strong> videoaulas disponíveis</span><span><strong>8</strong> materiais de apoio</span></div></div><div class="lib-hero-art">${cover(current())}<span>APRENDA · REVISE · PRATIQUE</span></div></section>
    <div class="lib-tabs-row"><div class="lib-tabs" id="libraryTabs" role="tablist" aria-label="Tipo de conteúdo">${[
      ["modulos", "Módulos", "layers"],
      ["instagram", "Instagram", "activity"],
      ["pdf", "PDF", "note"],
    ]
      .map(
        ([id, label, symbol]) =>
          `<button type="button" id="libraryTab-${id}" data-tab="${id}" role="tab" aria-controls="libraryPanel" aria-selected="false" tabindex="-1">${icon(symbol)}${label}<span>${id === "instagram" ? catalog.instagram.length : 8}</span></button>`,
      )
      .join(
        "",
      )}</div>${catalog.canManage ? '<button class="learn-button learn-button-secondary lib-manage" data-manage type="button">Gerenciar conteúdos +</button>' : ""}</div>
    ...
    $("librarySearch").addEventListener("input", renderContent);
    $("libraryFilter").addEventListener("change", renderContent);
    $("libraryTabs").addEventListener("keydown", (e) => {
      const tabs = ["modulos", "instagram", "pdf"],
  ...
  function renderTab() {
    ...
    $("libraryPanel").setAttribute("aria-labelledby", "libraryTab-" + tab);
    $("libraryFilter").hidden = tab !== "modulos";
    $("librarySearch").placeholder =
      tab === "instagram"
        ? "Buscar vídeo ou assunto"
        : "Buscar módulo ou assunto";
    renderContent();
  }
  ...
  function loadYoutube() {
    ...
    youtubeReady = new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          Error(
            "Não foi possível abrir o player. Use o link abaixo para assistir.",
          ),
        );
      }, 15000);
      ...
      script.onerror = () => {
        ...
        youtubeReady = null;
        reject(
          Error("O player está indisponível. Abra o vídeo pelo link abaixo."),
        );
      };
  ...
  async function startVideo() {
    const m = current(),
      media = Media.parse(m.url);
    if (!media || !m.published) return;
    stopPlayer();
    playingContent = m;
    const mount = $("libraryMedia");
    const position = videoProgress(m)?.completed
      ? 0
      : videoProgress(m)?.position || 0;
    mount.innerHTML =
      '<div class="lib-player-wait" role="status">Carregando videoaula…</div>';
    try {
      if (media.provider === "file") {
        mount.innerHTML =
          '<video controls playsinline preload="metadata" aria-label="Videoaula"></video>';
        ...
        video.addEventListener("pause", () => checkpoint());
        video.addEventListener("ended", () => checkpoint(true));
        video.addEventListener("error", () => {
          if ($("videoError")) {
            $("videoError").hidden = false;
            $("videoError").textContent =
              "Não foi possível reproduzir este arquivo. Tente abrir pelo link do vídeo.";
          }
        });
      } else {
        const YT = await loadYoutube();
        if (playingContent !== m || !mount.isConnected) return;
        mount.innerHTML = '<div id="youtubeLesson"></div>';
        const yt = new YT.Player("youtubeLesson", {
          ...
          events: {
            onReady: (e) => {
              if (playingContent === m) e.target.playVideo();
            },
            onStateChange: (e) => {
              if (playingContent === m && [0, 2].includes(e.data))
                checkpoint(e.data === 0);
            },
            onError: () => {
              if ($("videoError")) {
                $("videoError").hidden = false;
                $("videoError").textContent =
                  "Este vídeo não pôde ser reproduzido aqui. Tente abrir em outra aba.";
              }
            },
          },
        });
      ...
      if (playingContent === m && mount.isConnected) {
        mount.innerHTML = `<div class="lib-player-wait">${escape(e.message)}</div>`;
      }
    }
  }
  function checkpoint(completed = false) {
    const m = playingContent || current();
    if (!m?.published || !m.url) return;
    const before = videoProgress(m);
    let position = before?.position || 0,
      duration = before?.duration || 0;
    if (player && playingContent) {
      if (player.kind === "file") {
        position = player.element.currentTime;
        duration = player.element.duration;
      } else {
        position = player.element.getCurrentTime?.() || position;
        duration = player.element.getDuration?.() || duration;
      }
    }
    if (!Number.isFinite(duration) || duration <= 0) {
      if (!completed) return;
      duration = before?.duration || 1;
  ...
  function stopPlayer() {
    ...
    playingContent = null;
    if (previous) {
      if (previous.kind === "youtube") previous.element.destroy?.();
  ...
  async function download(id, button) {
    ...
    try {
      const response = await fetch(`/learning/materials/${id}.pdf`, {
        headers: headers(),
        credentials: "same-origin",
        cache: "no-store",
        signal: AbortSignal.timeout(30000),
      });
      if (!response.ok) throw Error("Não foi possível baixar o PDF agora.");
      const blob = await response.blob(),
        url = URL.createObjectURL(blob),
        a = document.createElement("a");
      a.href = url;
      a.download = `turma-do-primo-${id}.pdf`;
  ...
  $("libraryDialog").addEventListener("click", (e) => {
    if (e.target.closest("[data-close-dialog]")) closeDialog();
  });
  function instagram(id) {
    const m = catalog.instagram.find((v) => v.id === id),
      media = Media.parse(m?.url, "instagram");
    if (!media) {
      toast("Este vídeo ainda não tem um link disponível.");
      return;
    }
    openDialog(
      `<header><div><span class="learn-eyebrow">INSTAGRAM</span><h2 id="libraryDialogTitle">${escape(m.title)}</h2></div><button class="learn-icon-button" type="button" data-close-dialog aria-label="Fechar vídeo">×</button></header><iframe class="lib-instagram-embed" title="${escape(m.title)}" src="${escape(media.embed)}" allow="encrypted-media; fullscreen; picture-in-picture" referrerpolicy="strict-origin-when-cross-origin"></iframe><p>${escape(m.description)}</p><a class="learn-button" href="${media.url}" target="_blank" rel="noopener noreferrer">Ver no Instagram ↗</a>${instagramAction(m)}<p class="lib-player-note">A disponibilidade do vídeo depende da publicação no Instagram.</p>`,
    );
  }
  function manage(id = selected) {
    if (!catalog.canManage) return;
    const options = [...catalog.modules, ...catalog.instagram];
    editor = options.find((m) => m.id === id) || {
    ...
    $("contentForm").onsubmit = async (e) => {
      ...
      data.published = e.currentTarget.elements.published.checked;
      data.revision = editor.revision;
      data.action = editor.action || { kind: "none" };
      $("contentSave").disabled = true;
      $("contentError").hidden = true;
      try {
        await api(`/learning/content/${editor.id}`, {
          method: "PUT",
        ...
        toast(
          data.published
            ...
            : "Rascunho salvo. O conteúdo está oculto para os alunos.",
        );
      } catch (error) {
        if ($("contentError")) {
  ...
  $("modulesApp").addEventListener("click", (e) => {
    ...
    if (b.dataset.select) {
      ...
      const route = window.TurmaNavigation?.pathname || "/modulos";
      history.replaceState(history.state, "", `${route}#modulos/${selected}`);
      all(".lib-module-card").forEach((el) =>
        el.classList.toggle(
          "is-selected",
          el.querySelector("[data-select]").dataset.select === selected,
        ),
      );
      renderPlayer();
      if (matchMedia("(max-width:1100px)").matches)
        $("libraryPlayer").scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
    }
    if (b.dataset.pdf) navigate("pdf", b.dataset.pdf, true);
    if (b.dataset.lesson) navigate("modulos", b.dataset.lesson, true);
    if (b.dataset.read) {
      navigate("pdf", b.dataset.read);
      $("libraryReader").scrollIntoView({ behavior: "smooth", block: "start" });
      $("libraryReader").focus({ preventScroll: true });
    }
    if (b.dataset.download) download(b.dataset.download, b);
    if (b.hasAttribute("data-play")) startVideo();
    if (b.hasAttribute("data-complete")) {
      checkpoint(true);
      toast(
        S.status.pending
          ? "Conclusão marcada. Aguardando sincronização."
          : "Conclusão salva na sua conta.",
      );
    }
    if (b.dataset.instagram) instagram(b.dataset.instagram);
    if (b.hasAttribute("data-manage")) manage();
  });
