...
(() => {
  ...
  function loadExtraStudyModules(){if(!document.body?.classList.contains("study-home-page")||document.querySelector('script[data-extra-study-modules]'))return;const s=document.createElement("script");s.src=`/estudo-triangulacao-card.js?v=20260801-tri-1&t=${Date.now()}`;s.defer=true;s.dataset.extraStudyModules="1";document.head.appendChild(s)}
  ...
  function release(user){forceVisible();applyCustomIcons();loadMagnetoArtwork();loadExtraStudyModules();document.body.classList.add("protected-ready");document.getElementById("studyLoading")?.remove();document.dispatchEvent(new CustomEvent("turma:study-ready",{detail:{user}}))}
  async function start(){forceVisible();applyCustomIcons();loadMagnetoArtwork();loadExtraStudyModules();const token=getToken();if(!token){location.replace("/");return}try{const response=await fetch(`${location.origin}/me`,{headers:{Accept:"application/json",Authorization:`Bearer ${token}`},cache:"no-store"});const data=await response.json().catch(()=>({}));if(!response.ok||!data?.usuario)throw new Error("Sessão inválida");release(data.usuario)}catch(_){clearSession();location.replace("/")}}
